...
@Component({
  selector: 'app-movimiento-ingreso',
  templateUrl: './movimiento-ingreso.component.html',
  styleUrls: ['./movimiento-ingreso.component.scss']
})
export class MovimientoIngresoComponent implements OnInit {
  ...
  private paramsBusqueda = { idAlmacen: null, fiTipo: null, idMedicamento: null, numeroDocumento: null, feIni: null, feFin: null };
  private pageSize: number;
  private pagination: any;
  private paramsBusquedaNC = { idAlmacen: null, fiTipo: null, idMedicamento: null, numeroDocumento: null, feIni: null, feFin: null };
  ...
  private parametrosNC_Impresion = { idAlmacen: null, numeroDocumento: null, descripcionAlmacen: null, descripcionDocumento: null, noTipo: null, descripcionMedicamento: null, tipoFile: null, feIni: null, feFin: null, nuPagina: null, nuRegisMostrar: null };
  private comprFarmNCList: any = [];
  private movIngMedList: any = [];
  private parametros = { idAlmacen: null, fiTipo: null, idMedicamento: null, numeroDocumento: null, feIni: null, feFin: null, tipoFile: null, descripcionAlmacen: null, descripcionDocumento: null, noTipo: null, descripcionMedicamento: null, descripcionIni: null, descripcionFin: null };
  ...
  private placeDesc(opcion, _ngForm: any) {
    ...
    this.paramsBusquedaNC.numeroDocumento = null;
    this.filteredDispMedi = [];
    this.movIngMedList = [];
    this.comprFarmNCList = [];
    this.descripAutoComplete = (this.gruposMedicamentos.find(obj => obj['id'] === opcion))['valor'];
    this.paramsBusqueda = { idAlmacen: this.idAlmacen, fiTipo: null, idMedicamento: null, numeroDocumento: null, feIni: null, feFin: null };
    this.paramsBusquedaNC = { idAlmacen: this.idAlmacen, idMedicamento: null, numeroDocumento: null, fiTipo: null, feIni: null, feFin: null };
  }
  ...
  private buscarIngMedicamentoxSalida(numPagina?: number) {
    ...
    if (this.paramsBusqueda.numeroDocumento == '') {
      this.paramsBusqueda.numeroDocumento = null;
    }
    ...
    this._reporteFarmaciaService.listarMovimientoIngresoMedicamento(this.paramsBusqueda)
      .subscribe(data => {
        if (data.estado == 1) {
          this.movIngMedList = data.movimientoIngresoMedicamentoList;
        }
        else {
  ...
  private imprimirReportePromise(tipoFile) {
    let promise = new Promise((resolve, reject) => {
      ...
      this.parametros.numeroDocumento = this.paramsBusqueda.numeroDocumento;
      if (this.parametros.numeroDocumento == null || this.parametros.numeroDocumento == undefined || this.parametros.numeroDocumento == "") {
        this.parametros.descripcionDocumento = "--";
      } else {
        this.parametros.descripcionDocumento = this.parametros.numeroDocumento;
      }
      this.parametros.idMedicamento = this.paramsBusqueda.idMedicamento;
      ...
      this._reporteFarmaciaService.listarMovimientoIngresoMedicamento_Impresion(this.parametros).toPromise().then(data => {
        if (data.estado == 1) {
          if (tipoFile == 2) {
  ...
  private listarMovimientoporNumeroComprobante(numPaginaNC?: number) {
    ...
    if (this.paramsBusquedaNC.numeroDocumento == '') {
      this.paramsBusquedaNC.numeroDocumento = null;
    }
    ...
    this._reporteFarmaciaService.listarMovimientoIngresoMedicamento(this.paramsBusquedaNC)
      .subscribe(data => {
        if (data.estado == 1) {
          ...
          this.paramsBusquedaNC.numeroDocumento = "";
        }
        return true;
      },
        err => { console.error(err) },
  ...
  private imprimirPromiseNC(tipoFile) {
    let promise = new Promise((resolve, reject) => {
      ...
      this.parametrosNC_Impresion.numeroDocumento = this.paramsBusquedaNC.numeroDocumento;
      ...
      if (this.parametrosNC_Impresion.numeroDocumento == null || this.parametrosNC_Impresion.numeroDocumento == '') {
        this.parametrosNC_Impresion.descripcionDocumento = "Todas las pecosas";
      } else {
      ...
      this._reporteFarmaciaService.listarMovimientoIngresoMedicamento_Impresion(this.parametrosNC_Impresion).toPromise().then(data => {
        if (data.estado == 1) {
          if (this.comprFarmNCList.length > 0) {
            if (tipoFile == 2) {
              this.pdf = "data:application/pdf;base64," + data.imprimeFile;
