...
@Component({
  selector: 'app-examenes-auxiliares-laboratorio',
  templateUrl: './examenes-auxiliares-laboratorio.component.html',
  styleUrls: ['./examenes-auxiliares-laboratorio.component.scss']
})
export class ExamenesAuxiliaresLaboratorioComponent implements OnInit {
  private IPRESS: any;
  ...
  private RequestOrdenCabecera = { idTipoExamen: 1, idTipoDocumentoIdentidad: "0", nuDocumento: null, idOrdenExamenCabecera: null, idArea: 1 }
  private filtro2: any = [];
    ...
    for (let oec of this.ordenExamenCabecera) {
      ...
      if (idOrdenExamenCabecera == oec.idOrdenExamenCabecera) {
        ...
        this.filtro.forEach(item => {
          item.posicion = i;
          if (item.cita == undefined) {
            item.cita = {};
          }
          if (item.cita.fechaCita == null && item.estadoExamenDetalle == "P") {
          ...
          if (item.cita.fechaCita != null && item.estadoExamenDetalle == "P") {
            item.flag = true;
          } else if (item.cita.fechaCita != null && item.estadoExamenDetalle == "A") {
            item.flag = true;
          } else if (item.cita.fechaCita == null && item.estadoExamenDetalle == "A") {
            item.flag = true;
          }
  ...
  private generarCita() {
    ...
    if (this.vigenciaAtencion == 1 && this.vigenciaAcredita == 1) {
      const modalRef = this.modalService.open(AsignarCoberturaComponent, { size: 'sm', backdrop: 'static', keyboard: false });
      ...
      modalRef.result.then((result) => {
        ...
        this.idCobertura = reason;
        console.log("id: " + this.idCobertura);
      });
    } else {
      const modalRef = this.modalService.open(ParticularComponent, { size: 'lg', backdrop: 'static', keyboard: true });
      modalRef.componentInstance.idPersona = this.persona.dPersonaEncript;
    ...
    for (let item of this.filtro) {
      if (item.cita.fechaCita == null && item.estadoExamenDetalle == "P" && item.seleccionado == true) {
        this.filtro2.push(item)
      }
    }
    if (this.filtro2.length == 0) {
      this.toastr.error("Debe Seleccionar Una Orden de Examen o Solicitud de Examenes");
      return;
    }
  ...
  openModalHistorial() {
    ...
    modalRef.componentInstance.idTipoExamen = 1;
    modalRef.result.then((result) => {
      // this.idCobertura = result
      // console.log("id: " + this.idCobertura);
    }, (reason) => {
