...
import { ExamenesImagenologiaDeleteComponent } from './examenes-imagenologia-delete/examenes-imagenologia-delete.component';
import { ExamenesImagenologiaActualizaComponent } from './examenes-imagenologia-actualiza/examenes-imagenologia-actualiza.component';
...
@Component({
  selector: 'app-examenes-imagenologia',
  templateUrl: './examenes-imagenologia.component.html',
  styleUrls: ['./examenes-imagenologia.component.scss']
})

export class ExamenesImagenologiaComponent implements OnInit {
  //tabla1
    ...
    this._examenesApoyoService.obtenerCPTExamenes(this.cptExam.cptExamenList)
      .subscribe(data => {
        if (data.estado == 1) {
          ...
          if(this.RequestExamenes.ordenExamen)
          console.log(this.RequestExamenes.ordenExamen);
        } else {
          this.toastr.error(data.mensaje);
  ...
  private postOrdenExamenes() {
    ...
    this.RequestExamenes.ordenExamen.idActoMedico = this.idActoMedicoI;
    ...
    this.insertarExamenes.ordenExamen.idActoMedicoEncriptado = this.idActoMedicoI;
  ...
  private getobtenerOrdenExamenes() {
    // this.ordenExamen.idActoMedico = this.idActoMedicoI;
  ...
  modalEliminarOrdExamDetalle(idOrdenExamenDetalle){
    const dialogRef = this.dialog.open(ExamenesImagenologiaDeleteComponent, {
      autoFocus: false,
      hasBackdrop: true,
      maxWidth: '50%',
      height: '30%',
  ...
  modalActualizarOrdExaDetalle(idOrdenExamenDetalle){
    const dialogRef = this.dialog.open(ExamenesImagenologiaActualizaComponent, {
      autoFocus: false,
      hasBackdrop: true,
      minWidth: '70%',
      width: '1400px',
  ...
  private getObtenerImpresionExamenes() {
    ...
    this.impresionExamenes.idActoMedico = this.idActoMedicoI;
